...
const BOOKADD_URL = 'https://bookstorewebdeploy-production.up.railway.app/BookStore/import/add';
...
export default function StockOrder() {
    ...
    let maxStockId = 0;
    StockOrderData.forEach(order =>{
        const stockId = parseInt(order.StockId.substring(2));
                    ...
                    <table className="min-w-full">
                        <thead className="bg-header--lightcolor text-primary--color">
                            <tr>
                                ...
                                <th className='w-1/5 text-center py-2'>Mã nhân viên</th>
                        ...
                        <tbody className="bg-white">
                            {filteredData.map((item) => {
                                ...
                                <tr key={item.id} className="hover:bg-backgrond--color">
                                    ...
                                    <Link to={`/DonHang/${item.id}`} key={item.id} className="hover:underline">{item.id}</Link>
                                    </td>
                                    <td className="w-1/5 text-center py-2">{item.createBy}</td>
                                    <td className="w-1/5 text-center py-2">{formattedDate}</td>
                                    <td className="w-1/5 text-center py-2">{item.import_total} vnd</td>
                                    <td className={classNames("w-1/3 text-center py-2", TextColor(item.import_status))}>{ShippingStatus(item.import_status)}</td>
                                </tr>
                                );
                            })}
                        </tbody>
